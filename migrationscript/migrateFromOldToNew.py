...
for row in cur.fetchall():
    ...
    cur.execute(
        "SELECT guid FROM wp_posts WHERE post_type=\"attachment\" && guid != ' ' && post_parent = {0}".format(old_post_id))

    # File Document downloading starts here
    files = []
    ...
    for row in cur.fetchall():
        try:
            ...
            file_to_upload = open('tmp_file_to_upload', 'w')
            file_to_upload.write(response.read())
            file_to_upload.close()

            with open('tmp_file_to_upload', 'rb') as ftpup:
            ...
            print '    ' + str(err.code) + ' file not found'
